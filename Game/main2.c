...
const char *MapCreateStrFromNNTStr(map *Map, const char *Str, int Length, int *PoolI) {
    TRY(*PoolI + Length < _countof(Map->CharPool), "Out of memory");
    const char *Ret = &Map->CharPool[PoolI]; 
    CopyCharsWithNull(Ret, Str, Length); 
    *PoolI = *PoolI + Length + 1
    return Ret;
}
...
static int ReadMap(world *World, int MapI, const char *Path) {
    ...
    #define NEXT_STR() TRY(MapCreateStrFromNNTStr(Map, NEXT_RUN, Length, &PoolI));
